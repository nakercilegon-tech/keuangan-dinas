<!-- View Laporan Realisasi Pekerjaan (Tahap 9) -->
<div class="container-fluid py-3">
    ...
    <div class="card border-0 shadow-sm rounded-3 mb-4 no-print">
        <div class="card-body p-4">
            <div class="d-flex flex-wrap align-items-center justify-content-between gap-3 mb-3">
                <div>
                    <h4 class="fw-bold text-dark m-0"><i class="fa-solid fa-list-check text-indigo-600 me-2"></i>A. Laporan Realisasi Pekerjaan</h4>
                    <p class="text-muted fs-7 mb-0 mt-1">Monitoring status paket pekerjaan, pagu, nilai kontrak, realisasi pembayaran, sisa & penyedia TA 2026.</p>
                </div>
                ...
                <div class="d-flex align-items-center gap-2">
                    ...
                    <a href="/laporan/export_excel?type=pekerjaan" class="btn btn-sm btn-success rounded-pill px-3"><i class="fa-solid fa-file-excel me-1"></i> Export Excel</a>
                    <a href="/laporan/export_pdf?type=pekerjaan" target="_blank" class="btn btn-sm btn-danger rounded-pill px-3"><i class="fa-solid fa-file-pdf me-1"></i> Export PDF</a>
                </div>
            </div>

            <!-- Filter Parameter -->
            <form method="GET" action="/laporan/realisasi_pekerjaan" class="row g-2 pt-3 border-top">
    ...
    <div class="card border-0 shadow-sm rounded-3 p-4 bg-white" id="printableReport">
        ...
        <div class="text-center mb-4">
            <h5 class="fw-bold text-dark text-decoration-underline mb-1">LAPORAN REALISASI PEKERJAAN</h5>
            ...
            <table class="table table-bordered table-striped align-middle fs-7">
                <thead class="table-dark text-center uppercase fs-8">
                    <tr>
                        ...
                        <th class="align-middle">Pagu Paket (Rp)</th>
                        <th class="align-middle">Nilai Kontrak (Rp)</th>
                        <th class="align-middle">Total Pembayaran (Rp)</th>
                        <th class="align-middle">Sisa Kontrak (Rp)</th>
                        <th class="align-middle">% Efisiensi</th>
                        <th class="align-middle">Penyedia / Pelaksana</th>
                        <th class="align-middle">Status</th>
                    </tr>
                </thead>
                <tbody>
                    <?php if (!empty($list_pekerjaan)): ?>
                        <?php foreach ($list_pekerjaan as $row): ?>
                            <tr>
                                <td class="text-center font-mono fw-bold"><?= $row['no'] ?></td>
                                <td><?= $row['program'] ?></td>
                                <td><?= $row['kegiatan'] ?></td>
                                <td><?= $row['sub_kegiatan'] ?></td>
                                <td class="fw-bold text-indigo-700"><?= $row['paket'] ?></td>
                                <td class="text-end font-mono">Rp <?= number_format($row['pagu_paket'], 0, ',', '.') ?></td>
                                <td class="text-end font-mono fw-bold">Rp <?= number_format($row['nilai_kontrak'], 0, ',', '.') ?></td>
                                <td class="text-end font-mono text-emerald-700 fw-bold">Rp <?= number_format($row['total_pembayaran'], 0, ',', '.') ?></td>
                                <td class="text-end font-mono text-amber-700 fw-bold">Rp <?= number_format($row['sisa_kontrak'], 0, ',', '.') ?></td>
                                <td class="text-center font-mono font-bold text-purple-700"><?= number_format($row['persentase'], 2) ?>%</td>
                                <td class="fw-semibold"><?= $row['penyedia'] ?></td>
                                <td class="text-center"><span class="badge bg-success-subtle text-success border border-success-subtle px-2 py-1 rounded-pill fs-8"><?= $row['status'] ?></span></td>
                            </tr>
                        <?php endforeach; ?>
                    <?php endif; ?>
                </tbody>
